...
public class WalletClientTest : BaseClientTests {
    ...
    public async Task RequestApplePayPaymentSessionAsync_ResponseIsDeserializedInExpectedFormat() {
        ...
        // Assert
        response.EpochTimestamp.Should().Be(DateTimeOffset.FromUnixTimeMilliseconds(1555507053000).UtcDateTime);
        response.ExpiresAt.Should().Be(DateTimeOffset.FromUnixTimeMilliseconds(1555510653000).UtcDateTime);
